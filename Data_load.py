...
def read_UCI_data(num):
    if (num == 0):
        ...
        Train_data_t = np.load('NB15_trainData.npy')
        Train_label_t = np.load('NB15_trainLabel.npy')
        # Train_data_t = preprocessing.normalize(Train_data_t,axis=0)
    elif (num == 3):
        Train_data_t = np.load('ijcnn1_data.npy')
        Train_label_t = np.load('ijcnn1_label.npy')

        # min_max_scaler = preprocessing.MinMaxScaler()
        # Train_data_t = min_max_scaler.fit_transform(Train_data_t)
    elif (num == 4):
    ...
    if (num ==0 or num == 1 or num == 4):
        ...
        Test_data = np.load('NB15_testData.npy')
        Test_label = np.load('NB15_testLabel.npy')
        Test_label = (2 * (Test_label % 2)) - 1

...
def k_means_random_partition(train_data, train_label, E_node_n, k_centre_number, the_node_should_have_same_num_sample = False):
    ...
    if (the_node_should_have_same_num_sample):
        average_size = int(np.size(train_label) / E_node_n)

        for i in range(E_node_n):
            data_part[i], label_part[i], index_part[i] = shuffle(data_part[i], label_part[i], index_part[i] , random_state = 0)
        over_data = []
        over_label = []
        over_index = []
        over_ave = [int(0)]*E_node_n
        for i in range(E_node_n):
            over_ave[i] = int(np.size(label_part[i]) - average_size)
            if (over_ave[i] > 0):
                over_data.append(data_part[i][:over_ave[i],:])
                over_label.append(label_part[i][:over_ave[i]])
                over_index.append(index_part[i][:over_ave[i]])

                data_part[i] = np.delete(data_part[i], np.arange(0, over_ave[i]), 0)
                label_part[i] = np.delete(label_part[i], np.arange(0, over_ave[i]), 0)
                index_part[i] = np.delete(index_part[i], np.arange(0, over_ave[i]), 0)

        alloc_data = over_data[0]
        alloc_label = over_label[0]
        alloc_index = over_index[0]

        for i in range(1, len(over_data)):
            alloc_data = np.concatenate((alloc_data, over_data[i]))
            alloc_label = np.concatenate((alloc_label, over_label[i]))
            alloc_index = np.concatenate((alloc_index, over_index[i]))

        alloc_data, alloc_label, alloc_index = shuffle(alloc_data, alloc_label, alloc_index)

        for i in range(E_node_n):
            if (over_ave[i] < 0):
                add_data = np.copy(alloc_data[:np.abs(over_ave[i])])
                add_label = np.copy(alloc_label[:np.abs(over_ave[i])])
                add_index = np.copy(alloc_index[:np.abs(over_ave[i])])

                data_part[i] = np.concatenate((data_part[i], add_data))
                label_part[i] = np.concatenate((label_part[i], add_label))
                index_part[i] = np.concatenate((index_part[i], add_index))



                alloc_data = np.delete(alloc_data, np.arange(0, np.abs(over_ave[i])), 0)
                alloc_label = np.delete(alloc_label, np.arange(0, np.abs(over_ave[i])), 0)
                alloc_index = np.delete(alloc_index, np.arange(0, np.abs(over_ave[i])), 0)


        # for i in over_part:
        #     while (np.size (label_part[i]) > average_size):
        #         for j in range(E_node_n):
        #             if (np.size (label_part[j]) >= average_size):
        #                 continue
        #             if (np.size (label_part[i]) <= average_size):
        #                 break
